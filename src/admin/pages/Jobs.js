...
const Jobs = () => {
  ...
  const exportPDF = async () => {
    try {
      if (jobs.length === 0) {
        alert(t("No job data available to export."));
        return;
      }
      ...
      doc.text(t('Zahrat Beesan - Career Openings'), 14, 22);
      doc.setFontSize(10);
      doc.setTextColor(100);
      doc.text(`${t('Generated on:')} ${new Date().toLocaleString('en-GB', { timeZone: 'Asia/Amman' })}`, 14, 32);
      doc.text(t('Current active job vacancies and position details.'), 14, 38);

      const tableColumn = [t("Title"), t("Type"), t("Location"), t("Posted Date")];
      ...
      doc.save(`Zahrat Beesan_Jobs_${Date.now()}.pdf`);
    } catch (error) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      alert(t("Failed to save job"));
    }
  };

  const handleDelete = async (id) => {
    if (window.confirm(t("Delete this job opening?"))) {
      try {
        await axios.delete(`/api/careers/${id}`);
        fetchJobs();
      } catch (err) {
        alert(t("Failed to delete job"));
      }
    }
  };
  ...
  const inputStyle = {
    ...
    border: `1px solid ${colors.border}`, color: 'var(--admin-text)', outline: 'none', marginBottom: '15px'
  };
        ...
        <div style={{ position: 'fixed', inset: 0, backgroundColor: 'rgba(0,0,0,0.8)', zIndex: 3000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '20px' }}>
          <div style={{ backgroundColor: colors.bean, width: '100%', maxWidth: '500px', borderRadius: '20px', padding: '30px', border: `1px solid ${colors.border}` }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '25px' }}>
              <h2 style={{ color: 'var(--admin-text)', margin: 0 }}>{modalMode === 'add' ? t('Add New Job') : t('Edit Job')}</h2>
              <button onClick={() => setShowModal(false)} style={{ background: 'none', border: 'none', color: 'var(--admin-text)', cursor: 'pointer' }}><X /></button>
            </div>
            <form onSubmit={handleSubmit}>
              <label style={{ color: colors.crema, fontSize: '0.8rem', display: 'block', marginBottom: '5px' }}>{t('JOB TITLE')}</label>
              <input style={inputStyle} value={formData.title} onChange={e => setFormData({...formData, title: e.target.value})} required placeholder={t("e.g. Senior Barista")} />
              
              <div style={{ display: 'flex', gap: '15px' }}>
                <div style={{ flex: 1 }}>
                  <label style={{ color: colors.crema, fontSize: '0.8rem', display: 'block', marginBottom: '5px' }}>{t('TYPE')}</label>
                  <select 
                  ...
                  >
                    <option value="Full-time" style={{ backgroundColor: '#1a1a1a', color: 'var(--admin-text)' }}>{t('Full-time')}</option>
                    <option value="Part-time" style={{ backgroundColor: '#1a1a1a', color: 'var(--admin-text)' }}>{t('Part-time')}</option>
                    <option value="Contract" style={{ backgroundColor: '#1a1a1a', color: 'var(--admin-text)' }}>{t('Contract')}</option>
                    <option value="__custom__" style={{ backgroundColor: '#1a1a1a', color: 'var(--admin-text)' }}>{t('Other...')}</option>
                  </select>
                ...
                <div style={{ flex: 1 }}>
                  <label style={{ color: colors.crema, fontSize: '0.8rem', display: 'block', marginBottom: '5px' }}>{t('LOCATION')}</label>
                  <input style={inputStyle} value={formData.location} onChange={e => setFormData({...formData, location: e.target.value})} required />
                </div>
              </div>

              <label style={{ color: colors.crema, fontSize: '0.8rem', display: 'block', marginBottom: '5px' }}>{t('DESCRIPTION')}</label>
              ...
              <button type="submit" style={{ width: '100%', padding: '15px', backgroundColor: colors.crema, color: colors.espresso, border: 'none', borderRadius: '10px', fontWeight: 'bold', cursor: 'pointer' }}>
                {modalMode === 'add' ? t('Create Opening') : t('Update Job')}
              </button>
            </form>
          </div>
        </div>
      ...
      <div style={{ 
        ...
        <div style={{ display: 'flex', gap: '15px' }}>
          ...
          <button onClick={() => handleOpenModal('add')} style={{ backgroundColor: colors.crema, color: colors.espresso, border: 'none', padding: '14px 28px', borderRadius: '14px', fontWeight: 'bold', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '10px', boxShadow: '0 10px 20px rgba(196, 164, 132, 0.2)' }}>
            <Plus size={20} /> {t('Add Job')}
          </button>
        </div>
      </div>

      {loading ? (
        <div style={{ position: 'relative', zIndex: 1, color: colors.crema, padding: '0 20px' }}>{t('Loading jobs...')}</div>
      ) : (
        <div style={{ position: 'relative', zIndex: 1, display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(380px, 1fr))', gap: '30px' }}>
          {jobs.length > 0 ? jobs.map(job => (
            <div key={job.id} className="premium-row" style={{ 
              ...
              <div style={{ position: 'absolute', top: '30px', insetInlineEnd: '30px', display: 'flex', gap: '15px' }}>
              ...
              <div style={{ display: 'flex', gap: '20px', alignItems: 'center', marginBottom: '25px' }}>
                ...
                <div>
                  <h3 style={{ margin: 0, color: 'var(--admin-text)', fontSize: '1.4rem', fontFamily: 'serif' }}>{job.title}</h3>
            ...
            <div style={{ gridColumn: '1/-1', textAlign: 'center', padding: '100px', backgroundColor: colors.bean, borderRadius: '30px', border: `1px dashed ${colors.border}` }}>
              ...
              <h3 style={{ color: colors.crema }}>{t('No active job openings')}</h3>
              <p style={{ color: '#777' }}>{t('Click "Add Job" to start recruiting for Zahrat Beesan.')}</p>
            </div>
          )}
        </div>
      )}
