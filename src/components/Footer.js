...
export default function Footer({ onOpenPolicy }) {
  ...
  const handleSubscribe = async (e) => {
    ...
    try {
      ...
      if (res.ok) {
        setSubMsg(t('subscribeSuccess') || 'تم الاشتراك بنجاح في النشرة الملكية');
        setEmail('');
      } else {
        setSubMsg(t('error') || 'حدث خطأ، يرجى المحاولة لاحقاً');
      }
    } catch (err) {
      console.error(err);
      setSubMsg(t('error') || 'حدث خطأ، يرجى المحاولة لاحقاً');
  ...
  return (
    <footer className={styles.footer} style={{ direction: currentLang.dir || 'rtl' }}>
      ...
      <div className={styles.topAssuranceBanner}>
        <div className={styles.assuranceInner}>
          <div className={styles.assuranceItem}>
            ...
            <div>
              <strong className={styles.assuranceTitle}>{t('royalCraftsmanshipTitle') || 'حرفية وتطريز يدوي ملكي'}</strong>
              <p className={styles.assuranceDesc}>{t('royalCraftsmanshipDesc') || 'أقمشة كريب وحرير أصلية ومطرزات متقنة'}</p>
            </div>
          </div>
          ...
          <div className={styles.assuranceItem}>
            ...
            <div>
              <strong className={styles.assuranceTitle}>{t('globalShippingTitle') || 'شحن سريع ومباشر'}</strong>
              <p className={styles.assuranceDesc}>{t('globalShippingDesc') || 'توصيل محلي بالأردن ودولي لكافة دول العالم'}</p>
            </div>
          </div>
          ...
          <div className={styles.assuranceItem}>
            ...
            <div>
              <strong className={styles.assuranceTitle}>{t('giftPackagingTitle') || 'تغليف إهداء فاخر'}</strong>
              <p className={styles.assuranceDesc}>{t('giftPackagingDesc') || 'صناديق ملكية فاخرة مع بطاقات تهنئة راقية'}</p>
            </div>
          </div>
          ...
          <div className={styles.assuranceItem}>
            ...
            <div>
              <strong className={styles.assuranceTitle}>{t('authenticityGuaranteeTitle') || 'ضمان الجودة والأصالة'}</strong>
              <p className={styles.assuranceDesc}>{t('authenticityGuaranteeDesc') || 'علامة تجارية أردنية مسجلة ومرخصة رسمياً'}</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className={styles.inner}>
        ...
        {/* Column 1: Brand & Identity & Official Trust Seal */}
        <div className={styles.brandCol}>
          ...
          <p className={styles.brandDesc}>
            {t('footerBrandDesc') || 'زهرة بيسان — دار العبايات الملكية الفاخرة، نصمم لكِ إطلالات تأسر القلوب بلمسات الأصالة والوقار.'}
          </p>
          ...
          <div className={styles.onlineBadge}>
            ...
            <span>{t('globalOnlineStore') || 'متجر إلكتروني رسمي ومتاح عالمياً'}</span>
          </div>
        ...
        <div className={styles.col}>
          <h4 className={styles.colHeader}>{t('consultationContact') || 'التواصل والاستفسار'}</h4>
        ...
        <div className={styles.col}>
          <h4 className={styles.colHeader}>{t('newsletter') || 'النشرة الملكية'}</h4>
          <p className={styles.newsletterDesc}>
            {t('newsletterDesc') || 'اشتركي لتصلكِ أحدث الخصومات والعروض الحصرية وإعلانات الكولكشن الجديد.'}
          </p>
          ...
          <form onSubmit={handleSubscribe} className={styles.newsletterForm}>
            <div className={styles.inputWrap}>
              <input
                type="email"
                required
                placeholder={t('emailPlaceholder') || 'أدخلي بريدكِ الإلكتروني...'}
                value={email}
          ...
          <div className={styles.paymentSection}>
            <span className={styles.paymentTitle}>{t('securePaymentDesc') || 'بوابات الدفع الإلكتروني المعتمدة:'}</span>
            <div className={styles.paymentBadges}>
              ...
              <span className={styles.payChip}>stc pay</span>
